...
class RedisCommandsSpec extends CatsEffectSuite {
  ...
  test("xadd/xread parity"){
    ...
    action.run(redisConnection()).map{ resp => 
  ...
  test("xadd/xtrim parity"){
    ...
    action.run(redisConnection()).assertEquals(8)
  }
  ...
  test("xadd/xgroupread parity"){
    ...
    action.run(redisConnection()).assertEquals((Some(msg1), Some(msg2), Some(msg3)))
  }
